...
def list_item_main(data):
    ...
    for i, item in enumerate(data["items"]):
        ...
        if info:
            ...
            try:
                if float(rating) > 0:
                    extras.append(f"IMDb: {rating}")
            except (ValueError, TypeError):
                pass
            if extras:
                label = f"{label} [COLOR yellow]{' - '.join(extras)}[/COLOR]"
            info["title"] = label
        ...
        if VIEWXXX == 'false':
            ...
            if '18+' in label_ or 'xxx' in label_ or 'XXX' in label_ or 'cấp 3' in label or 'jav' in label_ or 'JAV' in label_ or '+' in label_ or 'sex' in label_ or 'SEX' in label_ or 'fuck' in label_ or '20+' in label_:
                ...
                path = 'plugin://plugin.video.vietmediaF2?action=browse&node_id=75'
        ...
        if item.get("info"):
            set_video_info(listItem, item["info"])
...
def list_item_main_cache(data):
    ...
    for item in items:
        ...
        set_video_info(list_item, item["info"])
