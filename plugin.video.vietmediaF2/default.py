...
import sys
from resources.utils import set_video_info
import urlquick
...
def process_url_input(url_input):
    ...
    if 'fshare' in url_input:
        ...
        if "folder" in url_input:
            file = 'plugin://plugin.video.vietmediaF2?action=play&url=https://www.fshare.vn/folder/' + f_id
            playable = False
            if len(name) == 0:
                name = 'Folder to play'
        else:
            file = 'plugin://plugin.video.vietmediaF2?action=play&url=https://www.fshare.vn/file/' + f_id + '[]' + subs
            playable = True
            if len(name) == 0:
                name = 'File to play'

    elif 'drive.google.com' in url_input:
        file = 'plugin://plugin.video.vietmediaF2?action=play&url=' + url_input
        playable = True
...
def PlayCode():
    ...
    if selected == -1:
        ...
        xbmc.executebuiltin("Container.Update(plugin://plugin.video.vietmediaF2, replace)")
        return {"content_type": "episodes", "items": []}
    elif selected == 0:
        return input_code_and_process(shorten_host)
    elif selected == 1:
        ...
        if len(parts) >= 2:
            link = parts[1].strip()
            if link.startswith('plugin://plugin.video.vietmediaF2?action=play&url='):
                real_url = link.replace('plugin://plugin.video.vietmediaF2?action=play&url=', '')
...
def SaveNumberHistory(name, link, thumb):
    ...
    if link.startswith('plugin://plugin.video.vietmediaF2?action=play&url='):
        link = link.replace('plugin://plugin.video.vietmediaF2?action=play&url=', '')
...
def watchedHistoryList():
    ...
    try:
        ...
        for entry in history:
            ...
            link = 'plugin://plugin.video.vietmediaF2?action=play&url=%s' % link
            playable = True
    ...
    try:
        ...
        for i in range(1, t):
            ...
            if '@' in name or 'folder' in href or "pastebin.com" in href or '"docs.google.com"' in href:
                playable = False
                link = 'plugin://plugin.video.vietmediaF2?action=play&url=%s' % href
            else:
                playable = True
                link = 'plugin://plugin.video.vietmediaF2?action=play&url=%s' % href
...
def fourshare_folder(url):
    ...
    for file in jdata["payload"]:
        ...
        item["is_playable"] = playable
        item["path"] = 'plugin://plugin.video.vietmediaF2?action=play&url=%s' % file["link"]
...
def searchHistory():
    ...
    if not check_history():
        ...
        items1 = [{
            ...
            'is_playable': False,
            'path': 'plugin://plugin.video.vietmediaF2?action=_timtrenfshare1_',
        ...
        items2 = [{
            ...
            'is_playable': False,
            'path': 'plugin://plugin.video.vietmediaF2?action=__removeAllSearchHistory__',
        ...
        for name in history:
            ...
            item["label"] = "[I]"+name+"[/I]"
            item["is_playable"] = playable
            item["path"] = 'plugin://plugin.video.vietmediaF2?action=_timtrenfshare1_&keyword=%s' % keyword
...
def search4sHistory():
    ...
    if not check_fshare_history():
        ...
        items1 = [{
            ...
            'is_playable': False,
            'path': 'plugin://plugin.video.vietmediaF2?action=_timtren4share1_',
            'thumbnail': 'https://i.imgur.com/F5582QW.png',
            'icon': 'https://i.imgur.com/F5582QW.png',
            'label2': '',
            'info': {
        ...
        items2 = [{
            'label': '[COLOR yellow][I]Xoá lịch sử tìm kiếm[/I][/COLOR]',
            'is_playable': False,
            'path': 'plugin://plugin.video.vietmediaF2?action=__removeAllSearchHistory4share__',
        ...
        for name in history:
            ...
            item["label"] = "[I]"+name+"[/I]"
            item["is_playable"] = playable
            item["path"] = 'plugin://plugin.video.vietmediaF2?action=_timtren4share1_&keyword=%s' % keyword
            item["thumbnail"] = "https://i.imgur.com/lwX6Kup.png"
            item["icon"] = "https://i.imgur.com/lwX6Kup.png"
            item["label2"] = ""
            item["info"] = {'plot': 'Kết quả tìm kiếm [COLOR yellow]'+name+'[/COLOR]'}
        ...
        def resolve_and_notify(link, subtitle_url=None):
            try:
                ...
                if use_external_player:
                    ...
                    dummy_item = xbmcgui.ListItem(path="special://home/addons/plugin.video.vietmediaF2/resources/dummy.mp4")
...
def parse_menu_data(data):
    ...
    for row in parsed_data['table']['rows']:
        ...
        path = row['c'][1]['v']
        if path:
            path = path.replace('plugin.video.vietmediaF2', 'plugin.video.vietmediaF')
            path = path.replace('plugin.video.vietmediaF', ADDON_ID)
        playable = row['c'][2]['v']
...
def FshareHost():
    ...
    items = [
        {
            "label": "thuviencine.live",
            "is_playable": False,
            "path": "plugin://plugin.video.vietmediaF2?action=browse&url=https://thuviencine.live/menu",
            "thumbnail": tvcn_icon,
            "icon": tvcn_icon,
            "label2": "thuviencine.live",
            "info": {"plot": "Dữ liệu được lấy từ thuviencine.live"},
            "art": {
                "thumb": tvcn_icon,
                "icon": tvcn_icon,
                "poster": tvcn_icon
        ...
        },
        {
            "label": "thuvienhd.top",
            "is_playable": False,
            "path": "plugin://plugin.video.vietmediaF2?action=browse&url=https://thuvienhd.top/menu",
            "thumbnail": tvhd_icon,
            "icon": tvhd_icon,
        ...
        },
        {
            "label": "hdvietnam.xyz",
            "is_playable": False,
            "path": "plugin://plugin.video.vietmediaF2?action=browse&url=https://hdvietnam/menu",
            "thumbnail": hdvn_icon,
            "icon": hdvn_icon,
    ...
    menu_items = [
        {
            ...
            "path": "plugin://plugin.video.vietmediaF2?action=quick_account_menu",
            "icon": password_icon,
            "info": {"plot": "Nhập nhanh tài khoản Fshare bằng mã QR hoặc Code"}
        },
        {
            "label": "Account Settings",
            "path": "plugin://plugin.video.vietmediaF2?action=__settings__",
            "icon": settings_icon,
            "info": {"plot": "Truy cập Addon settings"}
        },
        {
            "label": "Cập nhập thông tin tài khoản",
            "path": "plugin://plugin.video.vietmediaF2?action=account_fshare",
            "icon": refresh_icon,
            "info": {"plot": "Kiểm tra và xem trạng thái tài khoản"}
        },
        {
            "label": "Fshare Favourites",
            "path": "plugin://plugin.video.vietmediaF2?action=action=folderxxx",
            "icon": favourite_icon,
            "info": {"plot": "Xem thông tin các thư mục yêu thích"}
        },
        {
            "label": "Fshare Top Follow",
            "path": "plugin://plugin.video.vietmediaF2?action=top_follow_share",
            "icon": top_icon,
            "info": {"plot": "Danh sách các file được follow nhiều nhất."}
        },
        {
            "label": "Home Fshare Acc",
            "path": "plugin://plugin.video.vietmediaF2?action=home_fshare",
            "icon": home_icon,
            "info": {"plot": "Xem danh sách các file trong thư mục Home"}
        },
        {
            "label": "Xem thông tin tài khoản 4share",
            "path": "plugin://plugin.video.vietmediaF2?action=get_user_information_fourshare",
            "icon": fourshare_icon,
...
def go():
    ...
    if url == 'plugin://plugin.video.vietmediaF2/':
        url += '?action=menu'
        ...
        with open(lock_dir, "r") as f:
            ...
            for line in lines:
                ...
                if "/folder/" in line or "/d/" in line or "browse" in line or "docs.google.com" in line or "episodes" in line:
                    playable = False
                    line = line.replace("url=plugin://plugin.video.vietmediaF2?action=play&","")
                ...
                item["path"] = 'plugin://plugin.video.vietmediaF2%s' % link
    ...
    def process_url(url, site_name, receive_function, setting_key):
        ...
        if addon.getSetting(setting) == 'false':
            ...
            return
    def handle_url(url):
        if "thuviencine.com" in url:
            url = url.replace("thuviencine.com", "thuviencine.live")
            
        plugin_url = f"plugin://plugin.video.vietmediaF2?action=browse&url={url}"

        try:
            if "thuviencine" in url:
                process_url(plugin_url, 'thuviencine.live', tvcine.receive, 'thuviencine.live')
    ...
    if "thuviencine" in url:
        process_url(url, 'thuviencine.live', tvcine.receive, 'thuviencine.live')
