...
def is_main_menu():
    ...
    return ("?action=menu" in current_url or
            current_url.endswith("plugin.video.vietmediaF2/") or
            "main_menu=true" in current_url or
            ("?" not in current_url and current_url.endswith("plugin.video.vietmediaF2")) or
            current_url == "plugin://plugin.video.vietmediaF2")
...
def get_context_menu_for_main_menu(item, index):
    ...
    command = 'Addon.OpenSettings(plugin.video.vietmediaF2)'
