...
default_domain = "thuviencine.live"
...
def getlink(url, img, description, english_name):
    def getlink_tvcn(url, img, description):
        ...
        for link in links:
            ...
            item["path"] = 'plugin://plugin.video.vietmediaF2?action=browse&url=%s' % link
            item["thumbnail"] = img
            item["icon"] = img
            item["label2"] = name
...
def listMovie(url):
    ...
    def getlist(url):
        ...
        return data
    cache_key = hashlib.md5(url.encode()).hexdigest() + "_movie_list_v2"
    if cache_utils.check_cache(cache_key):
        cache_content = cache_utils.get_cache(cache_key)
        if cache_content:
            xbmc.sleep(500)
...
def receive(url):
    if "menu" in url:
        ...
        links = [f"plugin://plugin.video.vietmediaF2?action=browse&url={BASE_URL}timkiem/",
                f"plugin://plugin.video.vietmediaF2?action=browse&url={BASE_URL}movies/",
                f"plugin://plugin.video.vietmediaF2?action=browse&url={BASE_URL}tv-series/",
                f"plugin://plugin.video.vietmediaF2?action=browse&url={BASE_URL}top/",
                f"plugin://plugin.video.vietmediaF2?action=browse&url={BASE_URL}theloai",
                f"plugin://plugin.video.vietmediaF2?action=browse&url={BASE_URL}quocgia",
                f"plugin://plugin.video.vietmediaF2?action=browse&url={BASE_URL}chatluong"]
        items = []
        ...
        return data
    elif "tv-series" in url or "/movies/" in url or "/top/" in url or "vmf" in url:
        if "vmf" in url:
            url = url.replace("vmf","")
