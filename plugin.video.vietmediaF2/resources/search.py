...
def searchvmf(query):
    ...
    if not items_data:
        ...
        item_timfshare= {
                ...
                'is_playable': False,
                'path': 'plugin://plugin.video.vietmediaF2?action=__TIMFSHARE__&ref=ref&keyword='+query,
...
def timfshare(query):
    ...
    try:
        ...
        # Tắt thông báo lỗi và không thử lại để tránh spam màn hình và làm chậm addon
...
def searchtvhd(query):
        ...
        for link in item['links']:
            ...
            for ext in [".mkv", ".mp4", ".wmv", ".iso", ".ISO", ".ts"]:
                if ext in link["title"]:
                    ...
                    path = 'plugin://plugin.video.vietmediaF2?action=play&url='+furl
                    link_list_item = xbmcgui.ListItem(label=label)
                    set_video_info(link_list_item, {'Title': label,'Size': size})
    ...
    for x in data:
        ...
        for i in x['links']:
            ...
            if "file" in flink:
                link = ('plugin://plugin.video.vietmediaF2?action=play&url=%s' % flink)
                playable = True
            else:
                link = ('plugin://plugin.video.vietmediaF2?action=play&url=%s' % flink)
                playable = False
...
def searchFourshare(query,page):
    ...
    item_nextpage = {
            ...
            'is_playable': False,
            'path': 'plugin://plugin.video.vietmediaF2?action=_timtren4share1_&page='+str(nextpage)+'&keyword='+query,
