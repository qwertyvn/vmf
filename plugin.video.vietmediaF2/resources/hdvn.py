    ...
    def getlisthdvn(url):
        ...
        def process_thread_item(thread_item):
            ...
            if not sticky:
                ...
                item = create_item(name,False,f"plugin://plugin.video.vietmediaF2?action=browse&url={href}",image_path,"",image_path,"")
                return item
        ...
        item_nextpage = create_item(f"[COLOR yellow]Trang {next_page}[/COLOR]",False,f"plugin://plugin.video.vietmediaF2?action=browse&url={url_next_page}","https://i.imgur.com/yCGoDHr.png",f"Trang {next_page}","https://i.imgur.com/yCGoDHr.png","")
            ...
            if content:
                ...
                for link in links:
                    ...
                    if href and "fshare.vn" in href:
                        ...
                        if size_file is not None and size_file.isnumeric() and int(size_file) > 0:item = create_item(name,playable,f"plugin://plugin.video.vietmediaF2?action=browse&url={href}",image_path,'',image_path,size_file)
                        else:
                            name = "Files đã bị xoá"
                            item = create_item(name, playable, f"plugin://plugin.video.vietmediaF2?action=browse&url={href}", "https://i.imgur.com/1wxUwaE.png", '', "https://i.imgur.com/lrynDXs.png", size_file)
                        items.append(item)
                        count +=1
                        if count ==5:break
                #find more
            ...
            if content:
                ...
                for link in links:
                    ...
                    if href and "4share.vn" in href:
                        ...
                        item = create_item(name, playable, f"plugin://plugin.video.vietmediaF2?action=browse&url={href}", "https://i.imgur.com/ytoQpHG.png", '', "https://i.imgur.com/ytoQpHG.png", size_file)
                        items.append(item)
                #find more
                urls = re.findall(r'\[url=(.*?)\]', str(content))
                ...
                for href in share_urls:
                    ...
                    item = create_item(href, playable, f"plugin://plugin.video.vietmediaF2?action=browse&url={href}", "https://i.imgur.com/ytoQpHG.png", '', "https://i.imgur.com/ytoQpHG.png", size_file)
                    items.append(item)
                data = {"content_type": "episodes", "items": ""}
                data.update({"items": items})
                return data
    ...
    if divs:
        ...
        if threads:
            for thread in threads:
                ...
                href = h3.a.get('href')
                href = "https://www.hdvietnam.xyz/"+href
                path = f"plugin://plugin.video.vietmediaF2?action=browse&url={href}"
            ...
            if page:
                if next_page < int(max_page):
                    item_nextpage = create_item(f"[COLOR yellow]Trang {next_page}[/COLOR]",False,f"plugin://plugin.video.vietmediaF2?action=browse&url={next_page_url}","https://i.imgur.com/yCGoDHr.png",f"Trang {next_page}","https://i.imgur.com/yCGoDHr.png","")
                    items.append(item_nextpage)
            data = {"content_type": "episodes", "items": ""}
            data.update({"items": items})
            return data
...
def receive(url):
    ...
    if "menu" in url:
        ...
        links = [f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}timkiem/",
                f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}lastest/",
                f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}phimle",
                f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}phimbo",
                f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}phimhoathinh",
                f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}phimtheophanloai",
                f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}phimtailieu",
                f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}phim4K",
                f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/thu-vien-link-phim.150/"
                ]
        items = []
        ...
        item1 = create_item("[COLOR yellow]Phim bộ chất lượng mHD, SD[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/mhd-sd.104/", '','Phim bộ mHD, SD','','')
        ...
        item1 = create_item("[COLOR yellow]Phim WEB-DL, HDTV[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/web-dl-hdtv.271/", '','Phim webdl','','')
        item2 = create_item("[COLOR yellow]Bluray Remux[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/bluray-remux.324/", '','Phim bluray remux','','')
        item3 = create_item("[COLOR yellow]Phim mHD, SD[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/mhd-sd.77/", '','Phim mHD, SD','','')
        item4 = create_item("[COLOR yellow]Phim Bluray nguyên gốc[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/bluray-nguyen-goc.78/", '','Phim Bluray nguyên gốc','','')
        ...
        item1 = create_item("[COLOR yellow]WEB-DL, HDTV 4K[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/web-dl-hdtv-4k.344/", '','WEB-DL, HDTV 4K','','')
        item2 = create_item("[COLOR yellow]Bluray Remux 4K[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/bluray-remux-4k.345/", '','Bluray Remux 4K','','')
        item3 = create_item("[COLOR yellow]Bluray Nguyên Gốc 4K[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/bluray-nguyen-goc-4k.346/", '','Bluray Remux 4K','','')
        ...
        item1 = create_item("[COLOR yellow]Phim Tài Liệu - Thuyết Minh[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/phim-tai-lieu-thuyet-minh.347/", '','Phim Tài Liệu - Thuyết Minh','','')
        ...
        item1 = create_item("[COLOR yellow]Phim HH mHD, SD[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/mhd-sd.124/", '','Phim Tài Liệu - Thuyết Minh','','')
        ...
        item1 = create_item("[COLOR yellow]Phim có audio Việt[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/phim-co-audio-viet.265/", '','Phim có audio Việt','','')
        item2 = create_item("[COLOR yellow]Phim 3D[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/3d.110/", '','Phim 3D','','')
        item3 = create_item("[COLOR yellow]Phim cho iOS/Android[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/phim-cho-ios-android.157/", '','Phim cho iOS/Android','','')
        items = []
        items.insert(0, item3)
        items.insert(0, item2)
        items.insert(0, item1)
        ...
        if not check_history():  # Kiểm tra nếu lịch sử trống (lần đầu tiên nhấn tìm kiếm)
            ...
            items1 = [{
                ...
                'is_playable': False,
                'path': "plugin://plugin.video.vietmediaF2?action=browse&url=hdvietnam.xyz/tim_kiem_moi",
            ...
            items2 = [{
                ...
                'is_playable': False,
                'path': f"plugin://plugin.video.vietmediaF2?action=browse&url=hdvietnam.xyz/xoa_tk",
            ...
            with open(HISTORY_FILE, 'r') as file:
                ...
                if history:
                    for query in history:
                        item = create_item(query, False, f"plugin://plugin.video.vietmediaF2?action=browse&url=hdvietnam.xyz/tim_kiemkeyword={query}", "https://i.imgur.com/F5582QW.png", f"Tìm kiếm [COLOR yellow]{query}[/COLOR] trên HDVN", "https://i.imgur.com/F5582QW.png", "")
                        items.append(item)
        ...
        item1 = create_item("[COLOR yellow]WEB-DL, HDTV[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/web-dl-hdtv.277/", '','WEB-DL, HDTV','','')
        item2 = create_item("[COLOR yellow]Bluray nguyên gốc[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/bluray-nguyen-goc.342/", '','Bluray nguyên gốc','','')
        item3 = create_item("[COLOR yellow]Bluray Remux[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/bluray-remux.343/", '','Bluray Remux','','')
        item4 = create_item("[COLOR yellow]mHD, SD[/COLOR]", False, f"plugin://plugin.video.vietmediaF2?action=browse&url={url_hdvn}forums/mhd-sd.147/", '','mHD, SD','','')
