...
def login():
    ...
    payload = '{"app_key":"O5UTCpIlQez7xCjdzXzKDR+tAEnV51PosWxXIouT","user_email":"'+username+'","password":"'+password+'"}'
    headers = {'cache-control': "no-cache", 'User-Agent': 'thuviencine-FSWXQQ'}
    ...
    try:
        ...
        while True:
            ...
            for item in response:
                ...
                set_video_info(list_item, {'size': size})
...
def fsharegetFolder(url):
    ...
    try:
        ...
        for f_item in f_items:
            ...
            if f_item["type"] == "0":
                link = ('plugin://plugin.video.vietmediaF2?action=play&url=https://www.fshare.vn/folder/%s' % linkcode)
                playable = False
            else:
                link = ('plugin://plugin.video.vietmediaF2?action=play&url=https://www.fshare.vn/file/%s' % linkcode)
                playable = True
        ...
        if len(f_items) >= 100:
            ...
            nextpage = {
                ...
                "is_playable": False,
                "path": f'plugin://plugin.video.vietmediaF2?action=play&url={urllib.parse.quote_plus(next_url)}',
                "thumbnail": next_icon,
                "icon": next_icon,
                "label2": "",
...
def fshare_top_follow():
    ...
    for i in f_items:
        ...
        item["path"] = 'plugin://plugin.video.vietmediaF2?action=play&url=https://www.fshare.vn/folder/%s' % linkcode
        item["thumbnail"] = ''
...
def fshare_favourite(url):
    ...
    for i in f_items:
        ...
        if name == None:
            ...
            item["is_playable"] = False
            item["path"] = 'plugin://plugin.video.vietmediaF2?action=play&url=top_follow_share'
            item["thumbnail"] = ''
            item["icon"] = play_icon
            item["label2"] = ""
    ...
    try:
        ...
        nextpage_url = "plugin://plugin.video.vietmediaF2?action=play&url=" + (nextpage_url)
